...
with st.sidebar:
    ...
    if registered_players:
        ...
        st.warning("⚠️ No players found on Sheet1.")
        user_name = ""
...
# --- PAGE 1: KNOCKOUT PREDICTIONS (MAIN HOME PAGE) ---
if page == "Knockout Predictions":
    st.title("🏆 Knockout Round Predictions Portal")
    st.markdown("Select matches below to dynamically enter or adjust your score predictions before real-world kickoff times.")
    
    if not user_name:
        st.warning("👈 Please select your Profile Name from the sidebar dropdown menu to modify knockout picks.")
    else:
        st.success(f"Active Session: **{user_name}**")
        ...
        if not active_fixtures:
            st.info("📡 Waiting on finalized elimination match paths from the live API feed. Form boxes will load dynamically as teams qualify.")
        else:
            for stage_group in knockout_stages:
                ...
                for m in stage_matches:
                    m_id = str(m.get('id'))
                    h_name = m.get('homeTeam', {}).get('name') or "TBD (Home)"
                    a_name = m.get('awayTeam', {}).get('name') or "TBD (Away)"
                    is_locked = m.get('status') not in ["TIMED", "SCHEDULED"]
                    
                    clean_h = CLEAN_TEAM_MAP.get(standardize_string(h_name), h_name)
                    clean_a = CLEAN_TEAM_MAP.get(standardize_string(a_name), a_name)
                    ...
                    with st.container(border=True):
                        ...
                        with c1:
                            ...
                            h_score = st.number_input("Goals", min_value=0, max_value=20, value=default_h_score, key=f"h_{m_id}", disabled=is_locked)
                        ...
                        with c3:
                            ...
                            a_score = st.number_input("Goals", min_value=0, max_value=20, value=default_a_score, key=f"a_{m_id}", disabled=is_locked)
                        ...
                        if not is_locked:
                            if st.button("Save Prediction Entry", key=f"btn_{m_id}"):
                                try:
                                    ...
                                    st.toast(f"Saved: {clean_h} vs {clean_a} matrix updated!", icon="✅")
                                except Exception as ex:
                                    st.error(f"Error writing updates to sheet: {ex}")
                        else:
        ...
        try:
            ...
            if records:
                ...
                def calculate_live_user_score(row):
                    ...
                    group_mapping = {chr(65+i): f"Group {chr(65+i)}" for i in range(12)} # Groups A-L
                    ...
                    # Add Knockout Calculation (Outcome + Home Score + Away Score)
                    if not ko_df_all.empty and live_matches_list:
                        ...
                        for _, ko_pick in user_subset.iterrows():
                            ...
                            if api_match and api_match.get('status') == 'FINISHED':
                                ...
                                act_winner_side = api_score.get('winner')
                ...
                st.info("No pool entries found yet.")
        except Exception as e:
            st.error(f"Leaderboard Score Engine Exception: {e}")

# --- PAGE 3: CLOSED GROUP INSTRUCTIONS ---
elif page == "Group Predictions (Closed)":
    st.title("🏆 Group Stage Predictions Matrix")
    st.warning("🔒 Group stage submissions are locked. Points are currently calculating against live standings tables.")
...
# --- PAGE 4: RULES & CHAT FORUM ---
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        if submitted:
            if not user_name:
                st.error("Please select your name profile from the sidebar panel before typing.")
            elif not comment:
